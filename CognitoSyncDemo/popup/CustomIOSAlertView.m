...
- (id)init
{
    ...
    if (self) {
       ...
       buttonTitles = @[NSLocalizedString(@"Retry",@"") ];
...
- (UIView *)createContainerView
{
    if (containerView == NULL) {
        ...
        UIImageView *imageView = [[UIImageView alloc] initWithFrame:CGRectMake(self.containerView.frame.size.width, 10, 50,80)];
        
        
        [imageView setImage:[UIImage imageNamed:@"error"]];
        [containerView addSubview:imageView];
        
        
        
        UIButton *retry =[[UIButton alloc]initWithFrame:CGRectMake(100, 140, 100, 50)];
        
        [retry setTitle:NSLocalizedString(@"Retry",@"")  forState:UIControlStateNormal];
        [retry setTitleColor:[UIColor redColor] forState:UIControlStateNormal];
        [retry addTarget:self action:@selector(retry:) forControlEvents:UIControlEventTouchUpInside];
        
        
        [containerView addSubview:retry];
...
- (void)addButtonsToView: (UIView *)container
{
    ...
    for (int i=0; i<[buttonTitles count]; i++) {
      ...
      //[closeButton setTitle:[buttonTitles objectAtIndex:i] forState:UIControlStateNormal];
        [closeButton setTitle:NSLocalizedString(@"Retry",@"")  forState:UIControlStateNormal];
